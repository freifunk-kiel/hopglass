define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showHwImg(o, model) {
    ...
    if (o.thumbnail) {
      ...
      content.onerror = function() {
        console.log("picture not found - if you have configured hwimgs create an issue at https://github.com/Moorviper/meshviewer_hwpics/issues")
        document.getElementById("fallback-headline").style.display = "block"
        document.getElementById("hwimgdiv").style.display = "none"
      }
    }
  ...
  return function(config, el, router, d) {
    ...
    h2.id = "fallback-headline"
    el.appendChild(h2)

    var div = document.createElement("div")
    try {
      if (config.hwImg)
        div.appendChild(showHwImg(config.hwImg, d.nodeinfo.hardware.model))
      else
        div.appendChild(showHwImg({
    ...
    attributeEntry(attributes, div, d.nodeinfo.hostname)
